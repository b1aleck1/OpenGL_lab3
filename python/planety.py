...
def startup():
    """ Oblicza wierzchołki sfery (szablonu) """
    global VERTICES, N
...
def spin(angle):
    """ Obraca całą sceną (nasza kamera) """
...
def draw_sphere_model():
    """ Rysuje model sfery z globalnych wierzchołków """
    global VERTICES, N
...
# =================================================================
# NOWA FUNKCJA: Rysowanie orbity
# =================================================================
def draw_orbit(radius):
    """ Rysuje okrąg w płaszczyźnie XZ (orbita) """
    glBegin(GL_LINE_LOOP)  # GL_LINE_LOOP łączy ostatni wierzchołek z pierwszym
    glColor3f(0.5, 0.5, 0.5)  # Szary kolor dla orbity

    segments = 100  # Dokładność okręgu
...
def render(time):
    ...
    # 2. Rysujemy Orbitę 1
    glPushMatrix()
    draw_orbit(orbit_radius_1)
    glPopMatrix()

    # 3. Rysujemy Planetę 1
    glPushMatrix()
    orbit_angle_1 = time * orbit_speed_1
    planet_x_1 = orbit_radius_1 * math.cos(orbit_angle_1)
    planet_z_1 = orbit_radius_1 * math.sin(orbit_angle_1)
    ...
    orbit_radius_2 = 9.0  # Dalsza orbita
    orbit_speed_2 = 0.5  # Wolniejszy ruch

    # 4. Rysujemy Orbitę 2
    glPushMatrix()
    draw_orbit(orbit_radius_2)
    glPopMatrix()

    # 5. Rysujemy Planetę 2
    glPushMatrix()
    orbit_angle_2 = time * orbit_speed_2
    planet_x_2 = orbit_radius_2 * math.cos(orbit_angle_2)
    planet_z_2 = orbit_radius_2 * math.sin(orbit_angle_2)
...
def main():
    ...
    window = glfwCreateWindow(400, 400, "Lab 3: Symulacja (2 Planety)", None, None)
